...
## 2026-07-25 — 사업검토·조사 자료(2026) 공개 게시

- **추가**: 2019~2026년 위탁사업 원본 결과보고서 9종(PDF→md 변환)을 근거로 작성한 분석 문서 5종을 `doc/data/사업검토_2026/`에 신설.
  - `00_방송기술교육원_사업검토_종합요약.md` — 사업 연혁·예산·성과 종합 요약
  - `01_교육과정_분야별_기초심화_지역적합도_분류_및_2026년_평가.md` — 전 교육과정을 분야/수준/지역적합도로 재분류, 2026년 계획 평가
  - `02_커리큘럼_설계_프레임워크_초안_AI필수요건대응.md` — AI 필수요건 대응 커리큘럼 설계 프레임워크 초안
  - `03_외부기관_2025-2026_커리큘럼_조사.md` — RAPA·KOCCA·KCA 등 외부기관 커리큘럼 조사
  - `04_교육과정_상세_제목_강사_강의내용_2019-2026.md` — 전 교육과정 제목·강사·강의내용 상세 재구성
- **개인정보 마스킹**: 원본 변환본 9종 중 6종(2019~2023, 2025년 결과보고서, 2026년 정기총회자료)에 담당자·이사진 개인 연락처(휴대폰 38건 + 이메일 35건, 주로 2026년 정기총회자료에 집중)가 그대로 포함되어 있는 것을 발견. 공개 문서 사이트에 올리기 전 이름·소속·직책은 유지하고 전화번호·이메일만 `[비공개]`로 마스킹한 별도 공개본을 만들어 게시(원본은 비공개 소스 저장소 `docs/`에만 보관). 마스킹한 파일 첫머리에 "공개본 안내" 문구로 명시.
- **doc/index.html**: 새 섹션 "🔍 사업검토·조사 자료 (2026)" 추가 (분석 5건 + 원본 변환 9건, 총 14건). `doc/STATUS.md` 기준일 2026-07-25로 갱신 및 로컬 `docs/STATUS.md`와 최신 상태 동기화.
- **참고**: 같은 자료를 앱 내부에서도 볼 수 있도록 `app/src/app/docs`(문서 뷰어), `app/src/content/kobeta-docs/`(원본 그대로, 비공개 앱 내부용)가 소스 저장소에 함께 추가됨 — 이쪽은 마스킹되지 않은 원본이므로 공개 문서 사이트와는 별개로 취급.
